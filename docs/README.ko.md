...
**책상에서 벗어나도, 목소리로 작업을 이어간다.** Bosun은 녹음부터 전송까지, 목소리로 하는 요청을 ChatGPT에 연결하는 무료 MIT Mac 앱이다.
...
소파에 앉거나 거실에 누워서도 Mac에 실행된 ChatGPT에 다음 요청을 말한다. Bosun의 음성 감지를 켠 뒤 “Record, please”로 녹음을 시작하고, 내용을 말한 다음 “Send, please”로 전송한다. 키보드나 마우스에 손을 뻗을 필요가 없다.

Mac이 깨어 있고 대상 앱이 조작 가능한 상태여야 한다. 필요한 권한을 허용하고, 사용자의 위치에서 마이크가 목소리를 분명히 인식할 수 있어야 한다. Bosun은 Mac에서 동작하는 음성 연결 도구이며 인터넷 원격 접속 기능은 아니다.

연결된 AI가 Computer Use를 지원하는 환경에서는 Bosun이 목소리와 AI의 컴퓨터 작업을 이어준다. Bosun이 담당하는 것은 녹음과 요청 전송이며, 실제 작업 범위는 AI의 도구·권한·승인 절차에 달려 있다. 모든 ChatGPT 데스크톱 환경에서 컴퓨터 전체를 조작할 수 있다는 의미는 아니다.

## 현재 상태

현재 릴리스는 **0.8.14**이다. macOS 14 이상, Apple Silicon Mac과 기기 내 영어 음성 인식 자산이 필요하다. 메뉴는 한국어와 영어를 지원한다.

0.8.14 수정 코드로 긴 문장 전송이 성공한 것을 사용자가 확인했고 자동 검사도 통과했다. 배포용 DMG는 Apple 공증과 공증 티켓 첨부를 마쳤고 macOS Gatekeeper 검사도 통과했다. 전체 음성명령의 실제 동작과 다른 Mac에서의 새 설치는 아직 검증하지 않았다. [검증 상태](VALIDATION.md)를 참고한다.
...
새로운 기여자를 환영한다. 버그 제보, 음성 명령 테스트, 문서·번역 개선, 코드 수정 모두 참여할 수 있다. 한국어와 영어를 모두 받는다. 직접 쓰기 권한 없이도 저장소를 Fork해 수정하고 Pull Request를 보내면 관리자가 검토한다.

[기여 방법](CONTRIBUTING.ko.md) · [버그 제보와 개선 제안](https://github.com/guileschool/Bosun/issues)
